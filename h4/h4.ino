...
void applyMPGMovement(Axis* a, int detents) {
  ...
  // Block MPG when in HOLD state (paused)
  if (controllerState == STATE_HOLD) {
    Serial.println("BLOCKED: Controller in Hold state");
    return;
  }

  // Block MPG when controller is running
  if (controllerState == STATE_RUN) {
    Serial.println("BLOCKED: Controller running");
    return;
  }
  ...
  stepToContinuous(a, a->pos + a->pendingPos + delta);
}
...
void processMPGCommand(const String& cmd) {
  ...
  if (cmd == "?") {
    ...
    if (controllerState == STATE_HOLD) {
      ...
      controllerState = STATE_IDLE;  // Resume back to RUN, not IDLE
    }
    stopRequested = true;
    mpgEnabled = true;
    ...
    if (controllerState == STATE_HOLD) {
      Serial.println("resume received, continue program...");
      controllerState = STATE_IDLE;  // Resume back to RUN, not IDLE
    }
    stopRequested = false;
    mpgEnabled = false;
...
void IRAM_ATTR onAsyncTimer() {
  Axis* a = getAsyncAxis();
  if (controllerState != STATE_RUN || a->movingManually || a->disabled) {
    return;
...
void setRunningFromTask(bool running) {
  nextRunning = running;
  nextRunningFlag = true;
}

void setRunningFromLoop(bool running) {
  if (controllerState == STATE_RUN && running) {
    return;  // Already running, no-op
  }
  if (!running) {
    // Stopping: disable steppers and reset to IDLE
    setupIndex = 0;
    // Set state to IDLE when stopping (unless in HOLD)
    if (controllerState != STATE_HOLD) {
      controllerState = STATE_IDLE;
    }
  }
  ...
  if (running) {
    ...
    opSubIndex = 0;
    setupIndex = 0;
    // Set state to RUN when starting (unless in HOLD)
    if (controllerState != STATE_HOLD) {
      controllerState = STATE_RUN;
    }
  }
...
// Reference-counted stepper enable/disable. Only applies when needsRest is true (open-loop drivers
// that overheat if left energized). When needsRest is false (e.g. modern drivers like DM556T),
// this function returns immediately and the stepper stays energized to avoid losing steps.
...
void G04(const String& command) {
  ...
  while (millis() - startTime < dwellMs && controllerState == STATE_RUN) {
    taskYIELD();
...
bool handleGcode(const String& command) {
  ...
  if (op == 0 || op == 1) { // 0 also covers X and Z commands without G.
    // Block motion if in HOLD or ALARM state
    if (controllerState == STATE_HOLD) {
      Serial.println("error: motion not allowed in Hold state");
      return false;
    }
    if (controllerState == STATE_ALARM) {
      Serial.println("error: motion not allowed in Alarm state");
      return false;
    }

    // Transition to RUN state when starting motion from IDLE
    if (controllerState == STATE_IDLE) {
      controllerState = STATE_RUN;
    }
...
bool handleG33(const String& command) {
    ...
    setRunningFromLoop(true);
    
    // Wait for threading to complete
    while (controllerState == STATE_RUN) {
        checkForStopCommand();
        delay(10);
        if (controllerState != STATE_RUN) break;
    }
...
bool handleG92(const String& command) {
  ...
  // Check for C (spindle/rotary) axis parameter
...
void discountFullSpindleTurns() {
  ...
  if (dupr != 0 && !stepperIsRunning(&z) && (mode == MODE_NORMAL)) {
...
void processSpindleCounter() {
  ...
  // Always calculate RPM for status display
